...
fm <- lmercens(Surv(Reaction) ~ Days + (1|Subject), data = sleepstudy2, REML=DO_REML,
         start = c(260, 5, 3, 2), control = lmerControl(optimizer = "Nelder-Mead"))
...
The between-subject variance is estimated as `r sprintf("%4.f", exp(2*fm$par[3]))`
while the residual variance as `r sprintf("%4.f", exp(2*fm$par[4]))`.
