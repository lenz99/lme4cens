...
### Simple regression
We start with a single predictor `yearsmarried`.
```{r affairs_simple_lmcens}
fm.simple.aff <- lmcens(Surv(affairs, event, type = "left") ~ yearsmarried, data = Affairs)
summary(fm.simple.aff)
```
...
We compare the results with the output of `censReg`, using the default Nelder-Mead optimization method.
...
The coefficients are vastly different, although the log-likelihood is similar.

We use the BFGS-optimization method for the same model:
```{r affairs_multiple_censReg}
...
We look into the log-likelihood contributions per observations between the two models with coefficients 
as estimated by `lmcens`. We need to sort them as appraently the order is different.
...
When we start our `lmcens` with the parameter estimates of `censReg` (rounded on 1 digit) we get:
