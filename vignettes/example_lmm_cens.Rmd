...
Manual steps go like this:

```{r ex2_sleepstudy2_lme4cens_manual}

sleepstudy2 %>% 
  lFormula(Surv(Reaction3, time2 = Reaction3,
                event = event3, type = "interval") ~ Days + (1|Subject),
           data = ., REML=FALSE) -> 
  lForm
...
## optimize function
paramS <- c(260, 5, 3, 2)
##paramS <- c(233.53, 11.22, 4.24, 3.60 )

paramEst <- optim(par = paramS, fn = myDevFun_gh_f)
paramEst
```

We use the wrapper function from `lme4cens` with same fixed start values:
```{r ex2_sleepstudy2_lme4cens1}
lmercens_rInt(Surv(Reaction3, time2 = Reaction3, event = event3, type = "interval") ~ Days + (1|Subject), data = sleepstudy2, 
              start = c(260, 5, 3, 2), REML = FALSE)
```


Wrapper function with automatic start values estimated from data:
...
ll.censReg <- censReg::censReg(Reaction3 ~ as.numeric(Days), left = REACT_L3, right = REACT_R3, start = paramEst$par, logLikOnly = TRUE, nGHQ = 8L,
                               data = sleepstudy2.cr)
