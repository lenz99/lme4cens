...
### sleepstudy2 data
For data we use a censored version of the `sleepstudy`-data,
by applying the __fixed boundary left- and right-censoring__ (column `event3`) on the measurements of reaction time.
...
sleepstudy2 %>%
  ...
  dplyr::summarise_(`mean~Reaction~` = ~ mean(Reaction3), `median~Reaction~` = ~ median(Reaction3)) %>% 
...
* `r paramSL$theta` for random effects _and_
* `r paramSL$fixef` for fixed effects

The model fit is: 
```{r ex2_sleepstudy2_lme4cens1, echo = VERBOSE > 0}

# devFun <- lmercens(Surv(Reaction3, time2 = Reaction3, event = event3, type = "interval") ~ Days + (1|Subject),
#                         data = sleepstudy2, REML = FALSE, devFunOnly = TRUE)
# devFunGrad <- attr(devFun, "grad")
# devFun(param = unlist(paramSL))
# devFunGrad(paramVect = unlist(paramSL))
# numDeriv::grad(func = devFun, x = unlist(paramSL))
...
The result (at least for the intercept term) depends on the start values.
...
) %>% knitr::kable(caption = "Log-likelihood contributions at __censReg__'s optimal values on sleepstudy2-data")
```
...
fm.survreg <- survreg(
  Surv(Reaction3, time2 = Reaction3, event = event3, type = "interval") ~ Days + frailty(Subject, distribution = "gaussian"),
